 ...
 */
package relacionesej4salacine;

import clasess.Cine;
import clasess.Espectador;
import clasess.Metodos;
import clasess.Pelicula;

public class Relacionesej4SalaCine {
    ...
    public static void main(String[] args) {
        ...
        p[0] = new Pelicula("Esperando la carrosa", 120, 18, "Hernesto");
        p[1] = new Pelicula("Avenger Endgame", 180, 16, "IronMan");
        p[2] = new Pelicula("Rapidos y furiosos 9", 130, 18, "Vin Diesel");
        p[3] = new Pelicula("Black Widow", 160, 18, "Scarlett");
        p[4] = new Pelicula("El padrino", 190, 15, "Carlo Changretta"); 
        Cine c = new Cine(8, 6, p[Metodos.generarNumeroAleatorio(0, 4)], 125, 1);
        int num_espectadores = 20;
        Espectador e;
        int fila = 0;
        char letra = 0;
               
        for (int i = 0; i < num_espectadores && c.espaciosLibres(); i++) {

            e = new Espectador(Metodos.nombres[Metodos.generarNumeroAleatorio(0, Metodos.nombres.length - 1)],
            Metodos.generarNumeroAleatorio(10, 30),
            Metodos.generarRealAleatorio(125, 500));
            System.out.println(e);
            do {

                fila = Metodos.generarNumeroAleatorio(0, 7);
                letra = (char) Metodos.generarNumeroAleatorio('A', 'A' + 5);

            } while (c.espaciosLibresporButaca(fila, letra));

            if (c.PuedeverlaPelicula(e)){
                e.pagar(c.getPrecio());
                c.Sentar(fila, letra, e);               
            }        
        }
           c.mostrarAsientos();
            System.out.println("");
